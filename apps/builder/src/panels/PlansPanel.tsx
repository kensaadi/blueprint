...
import { TIERS, getTierMeta, type TierMeta } from '../licensing/tiers';
      ...
      <div className="mt-3">
        {tier.cta === 'contact' ? (
          <button
            ...
            className="flex w-full items-center justify-center gap-[5px] rounded-lg border py-[7px] text-[12px] font-medium transition-colors"
            style={{
          ...
          <button
            ...
            className="w-full rounded-lg py-[7px] text-[12px] font-medium transition-colors"
            style={{
        ...
        <>
          <div className="mb-3 mt-6 flex items-baseline justify-between">
            ...
            <span className="text-[12px]" style={{ color: 'var(--bd-text-faint)' }}>
              Monthly · unlocks Builder features. Templates stay separate.
            </span>
          </div>

          <div className="flex gap-3 overflow-x-auto pb-2">
            {upgradeTiers.map((tier) => (
              <UpgradeCard key={tier.id} tier={tier} onBuy={onBuy} onContact={onContact} />
            ))}
          </div>
        </>
      )}
