/** `dateTime` atom — bound to @dashforge/tw DateTimePicker. */
...
export function TwDateTime({ name, label, placeholder, helperText, required }: Props) {
  ...
  if (!name) {
    return (
      <div className="rounded-md border border-red-300 bg-red-50 p-3 text-sm text-red-700">
        DateTime node is missing required <code>name</code> prop.
      </div>
    );
  }
  if (!inForm) {
